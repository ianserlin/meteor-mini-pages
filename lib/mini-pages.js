  ...
  var MeteorExtensions = {
    ...
    */
    ...
    /**
      ...
      Pass no arguments to retrieve the entire current page invocation.
      Pass a non-object key to retrieve a particular value from the current page invocation.
      Pass an object for the first parameter to set the current page invocation to that value.
      Pass a key and a value to set a particular value for a key in the current page
      invocation.

      @param [key] {mixed} Optional key to retrieve from the current page invocation.
      @param [value] {mixed} Optional value to store under key in the current page invocation.
      @attribute Meteor.currentPage
    */
    currentPage: function (key, value) {
      var returnValue, 
        invocation = Meteor.router.invocation();
      switch(arguments.length){
        ...
        case 1:
          if(_.isObject(key) && !_.isArray(key)){
            Meteor.router.invocation(key);
            returnValue = key;
          }else{
            returnValue = invocation[key];
          }
          break;
        default: /* 2 or more args */
          invocation[key] = value;
          Meteor.router.invocation(invocation);
          returnValue = invocation;
          break;
      }
      return returnValue;
